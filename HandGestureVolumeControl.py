...
while True:
    ...
    if len(lmList) != 0:
        ...
        cv2.circle(img, (x2, y2), 15, (255,165,0), cv2.FILLED)
        cv2.line(img, (x1, y1), (x2, y2), (255,255,240), 3)
        cv2.circle(img, (cx, cy), 15, (0, 0, 0), cv2.FILLED)

        length = math.hypot(x2-x1, y2-y1)
        print(length)

        # Hand Range 50 - 300 135, 0, 255
        # Volume Range -74 - 0
        ...
        if length < 50:
            cv2.circle(img, (cx, cy), 15, (135, 0, 255), cv2.FILLED)
    ...
    cv2.putText(img, f'FPS: {int(fps)}', (10, 70),
                cv2.FONT_HERSHEY_PLAIN, 3, (51, 204, 255), 3)
